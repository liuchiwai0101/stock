      ...
      <main className="mx-auto flex w-full max-w-[1100px] flex-1 flex-col gap-5 px-4 py-5 sm:px-6 sm:py-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-white/45">
              Re-check the model suite, score prior BUY suggestions against newer quotes, and compare the last two
              scans
            </p>
          </div>
        ...
        {report ? (
          <div className="rounded-lg border border-sky-400/20 bg-sky-400/8 px-3 py-2 text-sm text-sky-100">
            ...
            {report.pending ? ` · ${report.pending} waiting for a newer quote snapshot` : ""}
            {" · live hit "}
        ...
        <div className="grid gap-3 sm:grid-cols-4">
          ...
          <Stat label="Universe" value={latest ? latest.scanMeta.total.toLocaleString() : "—"} />
          ...
          <CardContent className="overflow-x-auto">
            {scored.length === 0 ? (
              <p className="py-8 text-center text-sm text-white/45">
                No scored calls yet. Scan full US, then Evaluate now after quotes move forward.
              </p>
            ) : (
