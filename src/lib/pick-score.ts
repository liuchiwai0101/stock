...
export function toDailyPick(q: CompanyForecast, rank: number, pickScore: number): DailyPick {
  ...
  return {
    ...
    pickScore,
    modelBuyVotes: models.filter((m) => m.expectedReturn > 0).length,
    modelCount: models.length,
    recommendedWeight: q.recommendedWeight,
    signal: q.signal,
    liveReady: q.liveReady,
    horizon: q.backtest.horizon,
    modelLeans: Object.fromEntries(models.map((m) => [m.id, m.expectedReturn])),
  };
}
...
export function quotesAsOfDate(quotes: CompanyForecast[]): string {
  let latest = "";
  for (const q of quotes) {
    const d = q.history.at(-1)?.date;
